...
class Crawl(object):
    ...
    def __init__(self):
        ...
        self.crawls = {
            'Desktop': {
                ...
                'crawl_name': self.now.strftime('chrome-%b_12_%Y')
            },
            'Mobile': {
                'urls_file': 'urls/latest_crux_mobile.csv',
                'crawl_name': self.now.strftime('android-%b_12_%Y')
            }
        }
    ...
    def run(self):
        ...
        if not self.status['done']:
            ...
            retry_flow_control = pubsub_v1.types.FlowControl(max_messages=1)
            ...
            completed_flow_control = pubsub_v1.types.FlowControl(max_messages=1)
